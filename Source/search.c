...
TUNABLE(int QS_FUTILITY_THRESHOLD = 300);
...
static inline int16_t quiescence(thread_t *thread, searchstack_t *ss,
  ...
  while ((move = select_next(&picker)) != 0) {
    ...
    if (!is_loss(best_score)) {
      if (!is_direct_check(pos, &check_info, move) &&
          get_move_target(move) != previous_square &&
          !is_loss(futility_score) && is_move_promotion(move)) {
        if (moves_seen >= 3) {
          continue;
        }

        const int futility_value =
            futility_score + mvv[pos->mailbox[get_move_target(move)]];

        if (futility_value <= alpha) {
          best_score = MAX(best_score, futility_value);
          continue;
        }

        if (!SEE(pos, move, alpha - futility_score)) {
          best_score = MAX(best_score, MIN(alpha, futility_score));
          continue;
        }
      }
