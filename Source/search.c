...
TUNABLE(int ASP_WINDOW_FAIL_HIGH = 59);
TUNABLE(int HINDSIGH_REDUCTION_ADD = 2966);
TUNABLE(int HINDSIGH_REDUCTION_RED = 1953);
TUNABLE(int HINDSIGN_REDUCTION_EVAL_MARGIN = 85);
TUNABLE(int PROBCUT_MARGIN = 256);
TUNABLE(int PROBCUT_SEE_THRESHOLD = 100);
TUNABLE(int MO_SEE_HISTORY_DIVISER = 31);
TUNABLE(int MO_QUIET_HIST_MULT = 974);
TUNABLE(int MO_CONT1_HIST_MULT = 1019);
TUNABLE(int MO_CONT2_HIST_MULT = 988);
TUNABLE(int MO_CONT4_HIST_MULT = 1005);
TUNABLE(int MO_PAWN_HIST_MULT = 911);
TUNABLE(int MO_CAPT_HIST_MULT = 1230);
TUNABLE(int MO_MVV_MULT = 14982);
TUNABLE(int MO_CHECK_SEE = 7768);
TUNABLE(int MO_QUIET_SEE = 98);
TUNABLE(int HISTORY_PRUNING_MARGIN = 2567);
TUNABLE(int BNFP_MARGIN = 152);

TUNABLE(int QUIET_HISTORY_MALUS_MAX = 1046);
TUNABLE(int QUIET_HISTORY_BONUS_MAX = 1169);
TUNABLE(int QUIET_HISTORY_BASE_BONUS = 10);
TUNABLE(int QUIET_HISTORY_FACTOR_BONUS = 159);
TUNABLE(int QUIET_HISTORY_BASE_MALUS = 7);
TUNABLE(int QUIET_HISTORY_FACTOR_MALUS = 272);
TUNABLE(int QUIET_HISTORY_MAX_TT = 1478);
TUNABLE(int QUIET_HISTORY_TT_FACTOR = 130);
TUNABLE(int QUIET_HISTORY_TT_BASE = 76);

TUNABLE(int CAPTURE_HISTORY_MALUS_MAX = 882);
TUNABLE(int CAPTURE_HISTORY_BONUS_MAX = 1541);
...
TUNABLE(int CAPTURE_HISTORY_FACTOR_MALUS = 260);
TUNABLE(int CAPTURE_HISTORY_QS_BONUS = 179);
TUNABLE(int CAPTURE_HISTORY_QS_MALUS = 259);

TUNABLE(int CONT_HISTORY_MALUS_MAX = 1339);
TUNABLE(int CONT_HISTORY_BONUS_MAX = 2507);
TUNABLE(int CONT_HISTORY_BASE_BONUS = 9);
TUNABLE(int CONT_HISTORY_FACTOR_BONUS = 189);
TUNABLE(int CONT_HISTORY_BASE_MALUS = 8);
TUNABLE(int CONT_HISTORY_FACTOR_MALUS = 227);

TUNABLE(int PAWN_HISTORY_MALUS_MAX = 885);
TUNABLE(int PAWN_HISTORY_BONUS_MAX = 1443);
TUNABLE(int PAWN_HISTORY_BASE_BONUS = 8);
TUNABLE(int PAWN_HISTORY_FACTOR_BONUS = 187);
TUNABLE(int PAWN_HISTORY_BASE_MALUS = 10);
TUNABLE(int PAWN_HISTORY_FACTOR_MALUS = 142);

TUNABLE(double LMP_MARGIN_WORSENING_BASE = 1.3823933277299942);
TUNABLE(double LMP_MARGIN_WORSENING_FACTOR = 0.4044738155328253);
TUNABLE(double LMP_MARGIN_WORSENING_POWER = 1.6514109724420354);
TUNABLE(double LMP_MARGIN_IMPROVING_BASE = 2.962567719064795);
...
TUNABLE(double NODE_TIME_MIN = 0.5465877131241242);

TUNABLE(double EVAL_TIME_ADDITION = 1.189118475863216);
TUNABLE(double EVAL_TIME_MULTIPLIER = 0.05603047678266344);

TUNABLE(int SEEPieceValues[] = {67, 326, 320, 603, 1653, 0, 0});
...
static inline int16_t negamax(thread_t *thread, searchstack_t *ss,
  ...
  int16_t probcut_beta = MIN(beta + PROBCUT_MARGIN, MATE_SCORE - 1);
  ...
  if (!pv_node && !in_check && !ss->excluded_move && depth >= PROBCUT_DEPTH &&
    ...
    const int depth_base =
        depth - PROBCUT_SHALLOW_DEPTH - (ss->static_eval - beta) / 289;
    ...
    while ((move = select_next(&probcut_picker)) != 0) {
      ...
      uint8_t pc_depth = clamp(
          depth_base - clamp((probcut_score - probcut_beta - 50) / 300, 0, 3),
          0, depth - 1);

      uint8_t base_pc_depth = clamp(depth_base, 0, depth - 1);

      int16_t ada_beta =
          clamp((probcut_beta + (base_pc_depth - pc_depth) * 300),
                -MATE_SCORE + 1, MATE_SCORE - 1);

      // If qsearch doesn't fail high, try a deeper search
      if (probcut_score >= probcut_beta && pc_depth > 0) {
        probcut_score = -negamax(thread, ss + 1, -ada_beta, -ada_beta + 1,
                                 pc_depth, !cutnode, NON_PV);
        if (probcut_score < ada_beta && probcut_beta < ada_beta) {
          pc_depth = base_pc_depth;
          probcut_score =
              -negamax(thread, ss + 1, -probcut_beta, -probcut_beta + 1,
                       pc_depth, !cutnode, NON_PV);
        } else {
          probcut_beta = ada_beta;
        }
      }
      ...
      if (probcut_score >= probcut_beta) {
        ...
        write_hash_entry(tt_entry, pos, ply, probcut_score, raw_static_eval,
                         pc_depth + 1, move, HASH_FLAG_LOWER_BOUND, ss->tt_pv);
