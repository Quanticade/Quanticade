...
static inline int16_t negamax(thread_t *thread, searchstack_t *ss,
  ...
  searchstack_t *parent = ss - 1;

  if (!ss->excluded_move && !root_node && !in_check && parent->move &&
      !(get_move_capture(parent->move) || is_move_promotion(parent->move)) &&
      parent->static_eval != NO_SCORE) {
    const int parent_gain = -parent->static_eval - ss->static_eval;
    const int bonus = clamp(parent_gain * 10, -2011, 1965) + 970;
    update_quiet_history(thread, parent, parent->move, bonus);
  }
