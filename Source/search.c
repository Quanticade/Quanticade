...
int SEE_QUIET = 41;
int SEE_CAPTURE = 29;
int SEE_HISTORY_DIVISOR_QUIET = 40;
int SEE_HISTORY_DIVISOR_CAPTURE = 39;
int SE_PV_DOUBLE_MARGIN = 1;
int SE_DOUBLE_MARGIN = 0;
int SE_TRIPLE_MARGIN = 37;
int LMR_PV_NODE = 870;
int LMR_HISTORY_QUIET = 1108;
int LMR_HISTORY_NOISY = 1114;
...
int LMR_QUIET_HIST_DIV = 6726;
int LMR_CAPT_HIST_DIV = 8374;
...
static inline int16_t negamax(thread_t *thread, searchstack_t *ss,
  ...
  while ((move = select_next(&picker)) != 0) {
    ...
    if (!root_node && best_score > -MATE_SCORE) {
      ...
      int see_treshold;
      if (!get_move_capture(move)) {
        see_treshold = -SEE_QUIET * depth - ss->history_score / SEE_HISTORY_DIVISOR_QUIET;
      } else {
        see_treshold = -SEE_CAPTURE * depth * depth - ss->history_score / SEE_HISTORY_DIVISOR_CAPTURE;
      }
