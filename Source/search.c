...
void update_history_moves(thread_t *thread, int best_move, uint8_t depth) {
    ...
    if (score > best_score) {
      ...
      if (score > alpha) {
        ...
        if (get_move_capture(list_move) == 0) {
          // store history moves
          update_history_moves(thread, list_move, depth);
        }
        ...
        if (score >= beta) {
          ...
          if (get_move_capture(list_move) == 0) {
            ...
            thread->killer_moves[0][pos->ply] = list_move;
          }
