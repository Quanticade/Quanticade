          ...
          if (!(get_move_capture(best_move) || is_move_promotion(best_move))) {
            ...
            for (uint32_t i = 0; i < quiet_list->count; ++i) {
              ...
              if (move == best_move) {
                ...
                const int scaled_pawn_malus = (pawn_malus * scale) / 1024;
                const int scaled_quiet_malus = (quiet_malus * scale) / 1024;

                update_continuation_histories(thread, ss, move,
                                              scaled_cont_malus);
                update_pawn_history(thread, move, scaled_pawn_malus);
                update_quiet_history(thread, ss, move, scaled_quiet_malus);
              }
            }
          }
