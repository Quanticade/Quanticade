...
extern int SEE_QUIET_HISTORY_DIVISOR;
...
extern int SE_NOISY_DOUBLE_MARGIN;
...
extern int ASP_WINDOW_FAIL_HIGH;
...
extern int CAPTURE_HISTORY_QS_BONUS;
extern int CAPTURE_HISTORY_QS_MALUS;
...
#define SPSA_INT_MINMAX_RATE(VARIABLE, TUNABLE, MINIMUM, MAXIMUM, RATE_VALUE)  \
  add_int_spsa(STRINGIFY(VARIABLE), &VARIABLE, MINIMUM, MAXIMUM,               \
               RATE(RATE_VALUE), NULL, TUNABLE)
...
void init_spsa_table(void) {
  ...
  SPSA_INT_POISON(IIR_DEPTH_REDUCTION, 0);
  SPSA_INT_FUNC(SEE_QUIET, NULL, 1);
  SPSA_INT_FUNC(SEE_CAPTURE, NULL, 1);
  SPSA_INT_POISON(SEE_DEPTH, 0);
  SPSA_INT(SEE_QUIET_HISTORY_DIVISOR, 1);
  SPSA_INT(SEE_NOISY_HISTORY_DIVISOR, 1);
  SPSA_INT_POISON(SE_DEPTH, 0);
  SPSA_INT(SE_DEPTH_REDUCTION, 0);
  SPSA_INT_MINMAX_RATE(SE_DOUBLE_MARGIN, 1, -64, 160, 50);
  SPSA_INT(SE_PV_DOUBLE_MARGIN, 1);
  SPSA_INT_MINMAX_RATE(SE_NOISY_DOUBLE_MARGIN, 1, 0, 1600, 128);
  ...
  SPSA_INT(QUIET_HISTORY_BASE_BONUS, 1);
  SPSA_INT(QUIET_HISTORY_FACTOR_BONUS, 1);
  SPSA_INT(QUIET_HISTORY_BASE_MALUS, 1);
