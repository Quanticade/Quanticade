...
void update_continuation_history_moves(thread_t *thread, searchstack_t *ss,
  ...
  for (uint32_t i = 0; i < quiet_moves->count; ++i) {
    if (quiet_moves->entry[i].move == best_move) {
      update_continuation_history(thread, ss - 1, best_move, bonus, depth, 1);
      update_continuation_history(thread, ss - 2, best_move, bonus, depth, 1);
      update_continuation_history(thread, ss - 4, best_move, bonus, depth, 1);
    } else {
