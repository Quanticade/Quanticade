...
void update_major_corrhist(thread_t *thread, int16_t static_eval, int16_t score,
                           uint8_t depth, uint8_t tt_flag) {
  if (!static_eval_within_bounds(static_eval, score, tt_flag)) {
    return;
  }
  int16_t bonus = calculate_corrhist_bonus(static_eval, score, depth);
  thread->major_correction_history[thread->pos.side]
                                  [thread->pos.hash_keys.major_key & 16383] +=
      scale_corrhist_bonus(
          thread->major_correction_history[thread->pos.side]
                                          [thread->pos.hash_keys.major_key &
                                           16383],
          bonus);
}
