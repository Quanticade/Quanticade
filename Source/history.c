...
static inline void update_continuation_history(thread_t *thread,
  ...
  int adjust = is_best_move ? clamped_bonus : -clamped_malus;
  thread->continuation_history[quiet][prev_piece][prev_target][piece][target] +=
      adjust - thread->continuation_history[quiet][prev_piece][prev_target]
                                           [piece][target] *
                   abs(adjust) / HISTORY_MAX;
}
...
int16_t get_conthist_score(thread_t *thread, searchstack_t *ss, int move) {
  uint8_t quiet = !get_move_capture(move) && !is_move_promotion(move);
  return thread->continuation_history[quiet][ss->piece][get_move_target(
