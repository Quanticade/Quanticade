...
double DEF_TIME_MULTIPLIER = 0.08474941943006492;
double DEF_INC_MULTIPLIER = 0.8516341906405389;
double MAX_TIME_MULTIPLIER = 0.7516668494928799;
double SOFT_LIMIT_MULTIPLIER = 0.7977839091758427;
...
void time_control(position_t *pos, thread_t *threads, char *line) {
  ...
  if ((argument = strstr(line, "depth"))) {
    ...
    if (limits.timeset) {
      ...
      if (movetime) {
        limits.max_time = limits.time;
        limits.hard_limit = threads->starttime + limits.max_time;
        limits.base_soft = 0x7fffffff;
        limits.soft_limit = threads->starttime + limits.base_soft;
      } else {
        const int64_t total =
            (limits.movestogo > 0)
                ? (int64_t)((double)limits.time / limits.movestogo + limits.inc)
                : (int64_t)MAX(1, limits.time + 50 * limits.inc - 50 * move_overhead);

        limits.base_soft =
            MIN(limits.time * 0.4193, total * 0.0575);
        limits.soft_limit = threads->starttime + limits.base_soft;
        limits.max_time = MIN(limits.time * 0.9221 - move_overhead, limits.base_soft * 5.9280) - 10;
        limits.hard_limit = threads->starttime + limits.max_time;
      }
    }
  }
