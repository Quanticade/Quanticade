...
TUNABLE(double DEF_TIME_MULTIPLIER = 0.09290182937090609);
TUNABLE(double DEF_INC_MULTIPLIER = 0.8482062866652279);
TUNABLE(double MAX_TIME_MULTIPLIER = 0.7487428006794964);
TUNABLE(double SOFT_LIMIT_MULTIPLIER = 0.7977713408347681);
...
void time_control(position_t *pos, thread_t *threads, char *line) {
  ...
  if ((argument = strstr(line, "depth"))) {
    ...
    if (limits.timeset) {
      ...
      if (movetime) {
        limits.max_time = MAX(1, limits.time);
