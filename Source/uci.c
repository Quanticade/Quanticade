...
double DEF_TIME_MULTIPLIER = 0.07137743961502185;
double DEF_INC_MULTIPLIER = 0.8477603982129455;
double MAX_TIME_MULTIPLIER = 0.742735468659853;
double SOFT_LIMIT_MULTIPLIER = 0.7692957086984504;
  ...
  while (1) {
    ...
    // parse UCI "ucinewgame" command
      ...
      for (int i = 0; i < thread_count; ++i) {
        ...
        memset(threads[i].minor_correction_history, 0,
               sizeof(threads[i].minor_correction_history));
        memset(threads[i].major_correction_history, 0,
               sizeof(threads[i].major_correction_history));
      }
    }
