...
static inline void white_accumulator_addsub(accumulator_t *accumulator,
...
static inline void white_accumulator_addsubsub(accumulator_t *accumulator,
                                               accumulator_t *prev_accumulator,
                                               uint8_t white_bucket,
                                               uint8_t piece1, uint8_t piece2,
...
static inline void white_accumulator_addaddsubsub(
    accumulator_t *accumulator, accumulator_t *prev_accumulator,
    uint8_t white_bucket, uint8_t piece1, uint8_t piece2, uint8_t piece3,
...
static inline void black_accumulator_addsub(accumulator_t *accumulator,
                                            accumulator_t *prev_accumulator,
...
static inline void black_accumulator_addsubsub(accumulator_t *accumulator,
                                               accumulator_t *prev_accumulator,
                                               uint8_t black_bucket,
                                               uint8_t piece1, uint8_t piece2,
                                               uint8_t piece3, uint8_t from1,
...
static inline void black_accumulator_addaddsubsub(
    accumulator_t *accumulator, accumulator_t *prev_accumulator,
...
static inline void accumulator_addsubsub(
    ...
    uint8_t piece3, uint8_t from1, uint8_t from2, uint8_t to3) {
  size_t white_idx_from1 = get_white_idx(piece1, from1);
  size_t black_idx_from1 = get_black_idx(piece1, from1);
  size_t white_idx_from2 = get_white_idx(piece2, from2);
  size_t black_idx_from2 = get_black_idx(piece2, from2);
...
static inline void
accumulator_addaddsubsub(accumulator_t *accumulator,
                         accumulator_t *prev_accumulator, uint8_t white_bucket,
                         uint8_t black_bucket, uint8_t piece1, uint8_t piece2,
                         uint8_t piece3, uint8_t piece4, uint8_t from1,
                         uint8_t from2, uint8_t to3, uint8_t to4) {
  size_t white_idx_from1 = get_white_idx(piece1, from1);
  size_t black_idx_from1 = get_black_idx(piece1, from1);
  size_t white_idx_from2 = get_white_idx(piece2, from2);
  size_t black_idx_from2 = get_black_idx(piece2, from2);
...
void white_accumulator_make_move(accumulator_t *accumulator,
  ...
  if (promoted_piece) {
    ...
    if (capture) {
      ...
      white_accumulator_addsubsub(accumulator, prev_accumulator, white_bucket,
                                  pawn, captured_piece, promoted_piece, from,
                                  to, to);
    } else {
      white_accumulator_addsub(accumulator, prev_accumulator, white_bucket,
                               pawn, promoted_piece, from, to);
    }
  }
    ...
    uint8_t captured_piece = mailbox[to];
    white_accumulator_addsubsub(accumulator, prev_accumulator, white_bucket,
                                captured_piece, moving_piece, moving_piece, to,
                                from, to);
  }
    ...
    case (g1):
      // move H rook
      white_accumulator_addaddsubsub(accumulator, prev_accumulator,
                                     white_bucket, R, moving_piece, R,
                                     moving_piece, h1, from, f1, to);
      break;
    ...
    case (c8):
      ...
      white_accumulator_addaddsubsub(accumulator, prev_accumulator,
                                     white_bucket, r, moving_piece, r,
                                     moving_piece, a8, from, d8, to);
      break;
    }
  } else {
    white_accumulator_addsub(accumulator, prev_accumulator, white_bucket,
                             moving_piece, moving_piece, from, to);
  }
}
...
void black_accumulator_make_move(accumulator_t *accumulator,
  ...
  if (promoted_piece) {
    ...
    if (capture) {
      uint8_t captured_piece = mailbox[to];
      black_accumulator_addsubsub(accumulator, prev_accumulator, black_bucket,
                                  pawn, captured_piece, promoted_piece, from,
                                  to, to);
    } else {
      black_accumulator_addsub(accumulator, prev_accumulator, black_bucket,
                               pawn, promoted_piece, from, to);
    }
  }
    ...
    case (g1):
      // move H rook
      black_accumulator_addaddsubsub(accumulator, prev_accumulator,
                                     black_bucket, R, moving_piece, R,
                                     moving_piece, h1, from, f1, to);
      break;
